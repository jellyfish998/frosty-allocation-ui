...
export default function FrostyAllocationUI() {
  const [available, setAvailable] = useState([405181, 447787, 630411]);
  const [percentages, setPercentages] = useState([10, 10, 80]);
  const [priority, setPriority] = useState("0,1,2");
  const [leaderTruck, setLeaderTruck] = useState(135410);
  ...
  return (
    <div className="min-h-screen bg-[#f7faff] p-4 md:p-10 text-[#1e3a8a] font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <Card className="bg-white/60 backdrop-blur rounded-2xl p-6 shadow h-full min-h-[300px]">
        ...
        <Card className="bg-white/60 backdrop-blur rounded-2xl p-6 shadow h-full">
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                value={leaderTruck}
                onChange={(e) => setLeaderTruck(Number(e.target.value))}
              />
            </div>
